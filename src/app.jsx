...
function EmailScanModal({onAddTasks,onClose}){
  return(
    <div style={{position:"fixed",inset:0,background:"#060a12fc",zIndex:1500,display:"flex",flexDirection:"column"}}>
      <div style={{background:"#0a0f1c",borderBottom:"1px solid #1e2d4a",padding:"14px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div>
          <div style={{fontSize:8,color:"#3B82F6",letterSpacing:2.5,textTransform:"uppercase",marginBottom:2}}>Inbox + Sent · 48hrs</div>
          <div style={{fontSize:17,fontWeight:900}}>📧 Email Scan</div>
        </div>
        <button onClick={onClose} style={{padding:"6px 14px",borderRadius:8,background:"#131e30",border:"1px solid #1e2d4a",color:"#64748B",fontSize:12,cursor:"pointer"}}>✕ Close</button>
      </div>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 32px",textAlign:"center"}}>
        ...
        <div style={{fontSize:16,fontWeight:900,color:"#F1F5F9",marginBottom:10}}>Claude.ai Session Required</div>
        <div style={{fontSize:12,color:"#475569",lineHeight:1.9,marginBottom:28,maxWidth:280}}>
          Email scanning uses your Microsoft 365 inbox via a live Claude.ai connection. It cannot run in the standalone app.
        </div>
        <div style={{background:"#0c1422",border:"1px solid #3B82F633",borderRadius:12,padding:"16px 20px",marginBottom:28,textAlign:"left",width:"100%",maxWidth:300}}>
          <div style={{fontSize:9,color:"#3B82F6",letterSpacing:1.5,textTransform:"uppercase",marginBottom:10}}>How to scan from Claude.ai</div>
          {[
            "Open claude.ai on your phone",
            "Go to The Bridge project",
            "Tell Claude: Scan my inbox for tasks",
            "Approve tasks — they stay in The Bridge"
          ].map((step,i)=>(
            <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",marginBottom:8}}>
              <div style={{width:18,height:18,borderRadius:"50%",background:"#3B82F6",color:"#fff",fontSize:9,fontWeight:900,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginTop:1}}>{i+1}</div>
              <div style={{fontSize:11,color:"#94A3B8",lineHeight:1.5}}>{step}</div>
            </div>
          ))}
        </div>
        <div style={{fontSize:10,color:"#374151",lineHeight:1.7}}>For now, use the <span style={{color:"#F1F5F9",fontWeight:700}}>⚡ Quick Capture</span> bar<br/>or the <span style={{color:"#F1F5F9",fontWeight:700}}>+ button</span> to add tasks manually.</div>
      </div>
    </div>
  );
}
